		...
		if( false == get_page_by_title( $cleared_title, 'OBJECT', 'story') ){
			...
			***************************************/
			...
			if ( $n_filename ){
				...
				//set filepath
				#$filepath = 'http://localhost/wordpress/wp-content/uploads/sites/2' . basename( $n_filename );
				$filepath = plugins_url() . '/importnews/' . basename( $n_filename );
				echo $filepath . '<br>';
				
				//prepary array for insertion
				$attachment = array(
					'post_mime_type' => $filetype['type'],
					'post_title' 	 => $n_filename,
					'post_content'   => '',
				...
				//insert attachment
				$thumb_id = wp_insert_attachment( $attachment, $n_filename, $post_id );
				//set metadata
				$thumb_data = wp_generate_attachment_metadata( $thumb_id, $n_filename );

				//attach metadata
				wp_update_attachment_metadata( $thumb_id, $thumb_data );
				//add alt text for accessibility
				update_post_meta( $thumb_id, '_wp_attachment_image_alt', $img_alt );
				//set the post thumbnail
				set_post_thumbnail( $post_id, $thumb_id );
				add_post_meta( $post_id, '_thumb_id', $thumb_id );
			} //end if image*/
